function LoadUser(source)
	...
	local result = MySQL:getResults(executed_query, {'username', 'password', 'banned', 'admin', 'money', 'kills', 'deaths', 'personalvehicle'})
	...
	if hasAccount(source) and not isLoggedIn(source) then
		...
		if(password ~= nil) then
			if(VerifyPasswordHash(password, Users[username]['password'])) then 
				...
				TriggerClientEvent('chatMessage', source, 'SYSTEM', {0, 255, 0}, "💡 Hold M for the player Menu and type /help for a list of commands.")
